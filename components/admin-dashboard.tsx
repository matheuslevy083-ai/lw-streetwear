...
function readFiles(files: FileList | null, callback: (images: string[]) => void) {
  ...
  ).then((images) => callback(images.filter(Boolean)))
}
...
export function AdminDashboard() {
  ...
  function saveProducts(nextItems = items) {
    ...
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(cleanProducts))
    window.dispatchEvent(new StorageEvent("storage", { key: STORAGE_KEY, newValue: JSON.stringify(cleanProducts) }))
